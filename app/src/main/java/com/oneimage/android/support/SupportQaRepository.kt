...
val starterAnswers = listOf(
    ...
    qa("account-profile-sync", "Account", 102, "Why does my profile say syncing?", "The app is waiting for your Firebase account profile to load.", "The Settings and Billing screens read your user profile from Firebase. If role, plan, status, or credits show Syncing, keep the app open briefly and confirm you are online. If it stays stuck, support needs the signed-in email and approximate time.", "profile", "syncing", "settings", "firebase"),
    ...
    qa("billing-service-model", "Billing", 201, "Why does the app use credits?", "Generation uses metered compute, storage, and service maintenance.", "OneStudio is an independent creative service. Image, video, and 3D generation consume metered resources, so credits are required to run tasks. Signing in alone does not start a subscription or purchase.", "credits", "service model", "compute"),
    qa("billing-plan-options", "Billing", 202, "What plans are shown in the app?", "Billing shows Basic, Pro, and Admin account states.", "Basic is shown as the free account level. Pro is shown as a monthly plan with priority RTC channel access, advanced style controls, and no ads. Admin accounts show unlimited generation access and no credit deduction for tasks.", "pro", "basic", "admin", "plan"),
    qa("billing-secure-checkout", "Billing", 203, "How is payment handled?", "The app indicates checkout is handled through Stripe.", "Billing shows secure checkout via Stripe. You must acknowledge the service model before upgrade actions are enabled. Subscription payments are handled outside the QA database.", "stripe", "checkout", "payment"),
    qa("billing-credits-used", "Billing", 204, "Why were credits used for my generation?", "Credits are reserved when a workflow starts and are tied to the selected tool.", "Credits are used when you start a generation workflow. If a result fails before the service accepts the task, the app should not treat it as completed. Check workflow history first, then contact support with the workflow name and approximate time if the balance looks wrong.", "credits", "balance", "refund", "generation"),
